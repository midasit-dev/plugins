...
from py_loaded import LOADED
from py_stiffness import STIFFNESS
### do not delete this import scripts ###
...
class IEHP :
  ...
  async def loadHinges(self, ElementValue, ComponentValue):
    ...
    await self.fillElasticK0(tableList.get("2"), ComponentValue)

    return json.dumps(tableList)

  async def fillElasticK0(self, rows, ComponentValue):
    """타입 4(Elastic)인 행의 K0 를 E * I 로 채운다. **지연 로딩**이다.

    K0 = E * I 는 요소의 재료·단면이 있어야 해서 표를 네 개 더 읽는다
    (IEHG / ELEM / MATL / ope-SECTPROP). 조회가 1초에서 3~5초로 늘어나므로,
    **화면에 타입 4 가 하나라도 있을 때만** 읽는다. 타입 3·5 로만 이뤄진
    모델은 이 비용을 전혀 내지 않는다.
    """
    if not rows : return
    pending = [r for r in rows
               if str(r["DATA"].get("K0_SOURCE") or "").startswith("ELEMENT_DEPENDENT_4")]
    if not pending : return

    await STIFFNESS.loadElements()
    if not STIFFNESS.elementsLoaded : return

    for row in pending :
      value, why = STIFFNESS.elasticK0(row.get("NAME"), ComponentValue)
      if value is None :
        row["DATA"]["K0_SOURCE"] = why
        continue
      # E * I 는 부호를 가리지 않는다.
      row["DATA"]["K0"] = [value, value]
      row["DATA"]["K0_SOURCE"] = why
  ...
  def toTableRow(self, ElementValue, ComponentValue, key, rawData):
    ...
    if NAME == "MULTLIN" : 
      ...
      # 탭2 는 alpha 가 K0 에 대한 비율이라 x축을 실단위로 만들려면 K0 가 필요하다.
      # 탭1 은 D_DATA 가 이미 실변위라 필요 없다.
      # K0 는 [정(+), 부(-)] 이고, 못 구하면 None -> 그래프가 정규화 축으로 그린다.
      if tableType == 0 :
        DATA["K0"], DATA["K0_SOURCE"] = STIFFNESS.resolve(
          propData, rawData.get("HINGE_TYPE"), ComponentValue, historyModel)
  ...
  def readSkeleton(self, data, historyModel, tableType, ComponentValue, hingeType = None):
    ...
    # 초기강성은 슬롯이 넷인데 그중 하나만 유효하다. 어느 것인지는 값이 아니라
    # INITSTIFFTYPE + PALPHADELTA + HINGE_TYPE 이 정한다 (py_stiffness).
    # 표는 K0_INPUT 을 보여 주고, 저장은 K0_SLOT 이 가리키는 자리에 되쓴다.
    # 슬롯은 힌지 타입만으로 정해진다 - 표에서 타입을 User 로 바꾼 행에도
    # 쓸 자리가 있어야 하기 때문이다. 편집 가능 여부는 타입이 정한다
    # (shared/initStiffCell).
    k0Slot = STIFFNESS.userSlot(hingeType)

    return {
      'SYMMETRIC': data.get("SYMMETRIC"),
      'INITSTIFFNESS': data.get("INITSTIFFNESS"),
      'K0_SLOT': k0Slot,
      'K0_INPUT': data.get(k0Slot),
      # 표가 초기강성 칸에 무엇을 쓸지 정하는 데 쓴다 (값 / 산정 방식 이름).
      'INITSTIFFTYPE': data.get("INITSTIFFTYPE"),
  ...
  def buildRecord(self, ElementValue, Component, tableType, value, key = None):
    ...
    # 신규 행은 조회 경로를 거치지 않아 K0_SLOT 이 없다. 힌지 타입만으로 정해지는
    # 값이라 여기서 채운다 - 비워 두면 사용자가 넣은 초기강성이 조용히 버려진다.
    if not DATA.get("K0_SLOT") :
      DATA["K0_SLOT"] = STIFFNESS.userSlot(newValue.get("HINGE_TYPE"))
  ...
  def writeSkeleton(self, Component, tableType, propData, DATA, modelPropName):
      ...
      **propData 를 제자리에서 고친다.** buildRecord 가 이미
      ...
      # 초기강성은 **읽을 때 고른 슬롯에만** 되쓴다. 반대쪽 슬롯에는 이전 힌지
      # 타입에서 쓰던 값이 남아 있어(인계서 6장), 거기에 쓰면 실제로 쓰이는 값은
      # 그대로인 채 안 쓰이는 값만 바뀐다.
      # 슬롯이 없으면(타입 4/5 등) 사용자가 못 고치는 자리이므로 원본을 보존한다.
      # 초기강성 타입. 표에서 바꿀 수 있으므로 그대로 되쓴다.
      stiffType = DATA.get("INITSTIFFTYPE")
      if stiffType is not None :
        propData["INITSTIFFTYPE"] = stiffType

      # 값은 **사용자 지정(3)일 때만** 쓴다. 그 밖의 타입은 CIVIL 이 계산하는
      # 값이라 슬롯을 건드리면 안 되고, 원본이 그대로 따라간다.
      k0Slot = DATA.get("K0_SLOT")
      k0Input = DATA.get("K0_INPUT")
      if stiffType == STIFFNESS.TYPE_USER and k0Slot and k0Input is not None :
        propData[k0Slot] = k0Input
      elif "INITSTIFFNESS" not in propData :
        # 신규 레코드는 TEMPLATE 에서 시작해 이 필드가 없다. 서버 기본값을 채운다.
