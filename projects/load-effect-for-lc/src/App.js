...
import { makeCombData } from "./utils";
import {
  VerifyUtil,
  VerifyDialog,
  GuideBox,
...
function Main() {
  ...
  const LcomListGridDef = React.useMemo(() => [
    {
      ...
      sortable: false,
      valueGetter: (params) =>
        `${params.row.key}${!params.row.isPending ? "*" : ""}`,
      flex: 0.1,
    },
    {
      field: "NAME",
    ...
    },
    {
      ...
      valueGetter: (params) =>
        typeValueOptions.find((type) => type.value === params.row.iTYPE).value,
    },
    {
      field: "DESC",
  ...
  const [isAddCombinationStatus, setIsAddCombinationStatus] =
    React.useState(false);
  const [isClickedLcomTableCell, setIsClickedLcomTableCell] =
    React.useState(false);
  const GoToStep2 = () => {
    if (combName === "Undefined") {
      enqueueSnackbar("No Load Combination selected.", {
        variant: "error",
  ...
  const [displayUnit, setDisplayUnit] = React.useState({
    force: "KN",
    dist: "M",
  });
      ...
      {openFormDlg === true ? (
        ...
        <GuideBox width="100%" center>
          <MoaStack spacing={2} padding={2}>
            ...
            {true && (
              <MoaStack
                direction={isPortrate ? "column" : "row"}
                justifyContent="center"
                spacing={1}
              >
                <MoaStack
                  direction="column"
                  width={isPortrate ? "100%" : 800}
                  sx={{
                    opacity: isAddCombinationStatus ? 0.5 : 1,
                  }}
                >
                  <Panel
                    width="inherit"
                  ...
                  >
                    ...
                    <GuideBox
                      ...
                      verSpaceBetween
                    >
                      <Scrollbars autoHide autoHeightMax="350px" autoHeight>
                        <MoaDataGrid
                      ...
                      <GuideBox width="100%" row center spacing={2}>
                        <Tooltip
                          title="Refresh Data From MIDAS Civil"
                          placement="top"
                        >
                      ...
                      <GuideBox width="100%" padding={1}>
                        <Scrollbars autoHide autoHeight autoHeightMax={"260px"}>
                          <MoaDataGrid
                            ...
                            experimentalFeatures={{ newEditingApi: true }}
                            hideFooter
                            onCellClick={({ id, field, value }) =>
                              handleCellSelect(id, field, value)
                            }
                          />
                        </Scrollbars>
                      </GuideBox>
                        ...
                        <GuideBox width="100%" height={415} spacing={1} column>
                          ...
                          >
                            <GuideBox width="100%" padding={1} spacing={2}>
                              <ElementList
                                element={prePostTableData.element}
                                onElementSelect={handleOnChange}
                              />
