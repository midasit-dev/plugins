...
export const DataLoader = async ({ user }) => {
  ...
  if (rawData[RESOLVE_DBNAME] === undefined) return [];

  const registeredItems = [];
  const dbData = rawData[RESOLVE_DBNAME];

  for (const value in dbData) {
    const targetData = dbData[value];
    const findResult = user.findIndex(
      (value) => value.NAME === targetData.NAME
    );
    if (findResult === -1) registeredItems.push(targetData);
...
export const DataRawLoader = async ({ user }) => {
  ...
  if (user.length > 0) {
    for (const value of user) {
      const findResult = registeredItems.findIndex(
        (registeredItem) => registeredItem.key === value.key
      );
      if (findResult === -1) {
