...
export const GetBeamforceData = async ({
  part,
  style,
  unit,
  element,
  ...
  if (hasError(rawData))
    throw new Error(
      "Cannot generate table data as there is no analysis result."
    );
  if (rawData?.TABLE_NAME === undefined) return [];
  ...
  for (const requestLcomItem of lcomData) {
    // [{NAME, FACTOR, }]
    ...
    if (requestLcomItem.NAME.includes("(CS)")) {
      ...
      const currentStageAddSteps =
        2 + (stageData[lastStageKey]?.ADD_STEP?.length | 0);
    ...
    if (result?.TABLE_NAME === undefined) continue;

    const resultData = result?.TABLE_NAME?.DATA || [];
