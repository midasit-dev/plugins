...
const App = () => {
  ...
  const getTaperedSection = useCallback(() => {
    ...
    if ("error" in Result) {
      enqueueSnackbar(Result.error, {
        ...
        style: { width: "220px", padding: 10, fontSize: 12 },
      });
      return;
    }
  ...
  useEffect(() => {
    checkPyScriptReady(() => getTaperedSection());
  }, [getTaperedSection]);
  ...
  const handleGenerateSection = () => {
    if (NameTag === "" || NameTag === undefined) {
      enqueueSnackbar("Please enter Name Tag", {
        variant: "error",
        autoHideDuration: 3000,
        style: { width: "220px", padding: 10, fontSize: 12 },
      });
    ...
    if (selectedList.length === 0) {
      enqueueSnackbar("Please select Section", {
        variant: "error",
        autoHideDuration: 3000,
        style: { width: "220px", padding: 10, fontSize: 12 },
      });
    ...
    const OrgSectionArray = Object.values(
      JSON.parse(JSON.stringify(SectionJson))
    );
    ...
    selectedSection.forEach((value: any) => {
      ...
      const newSection: any = OrgSectionArray.find(
        (section: any) => section.SECT_NAME === value
      );
    ...
    if ("error" in Result) {
      ...
      getTaperedSection();
      enqueueSnackbar("New Section Created", {
        variant: "success",
        autoHideDuration: 3000,
        style: { width: "220px", padding: 10, fontSize: 12 },
      });
  ...
  return (
    <GuideBox width={235} spacing={1} padding={1}>
      <GuideBox row width={220} horSpaceBetween verCenter>
        ...
        <IconButton onClick={handleRefresh} transparent>
      ...
      <Panel>
        <Box sx={{ width: 200, height: 200, overflow: "auto" }}>
          <List dense={true} disablePadding={true}>
            {sectionList.map((value: any, index: any) => {
              return (
                <ListItem
                  key={index}
                  secondaryAction={
                    <Check checked={sectionList[index].checked} />
                  }
            ...
            {sectionList.map((value: any, index: any) => (
              <Typography key={index} center></Typography>
            ))}
          </List>
        </Box>
      </Panel>
      ...
      <Button variant="outlined" width="220px" onClick={handleGenerateSection}>
        Generate
      </Button>
    </GuideBox>
  );
