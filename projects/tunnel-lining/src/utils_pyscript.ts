...
export function getSelectedElements(){
	return checkPyScriptReady(() => {
		const py_get_selected_elements = pyscript.interpreter.globals.get('py_select_node_list');
		const result = py_get_selected_elements();
		return JSON.parse(result);
	});
}

export function GenerateNodes(selectedelems:any, Modulus:any, Poisson:any, boundaryCondition:any){
	return checkPyScriptReady(() => {
		const py_generate_nodes = pyscript.interpreter.globals.get('py_generate_nodes');
		const result = py_generate_nodes(JSON.stringify(selectedelems), Modulus, Poisson, boundaryCondition);
		return JSON.parse(result);
	});
}