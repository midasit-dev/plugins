...
def horizontal_spectrum_parameter(pga_value,importance_factor,type, ground_type):
...
def vertical_spectrum_parameter(pga_value, importance_factor, type, ground_type):
    soil_factor, tb, tc, td = horizontal_spectrum_parameter(pga_value, importance_factor, type, ground_type)
    ag_value = pga_value * 0.10197 * importance_factor
    if type == "Type 1":
      avg_value = 0.90*ag_value
      tvb = 0.05
      tvc = 0.15
      tvd = 1.0
    elif type == "Type 2":
      avg_value = 0.45*ag_value
      tvb = 0.05
      tvc = 0.15
      tvd = 1.0
...
def horizontal_elastic_spectrum(ground_type, pga_value, importance_factor, type, damping_ratio, max_period):
    soil_factor, tb, tc, td = horizontal_spectrum_parameter(pga_value, importance_factor, type, ground_type)
    ag_value = pga_value * 0.10197 * importance_factor
...
def vertical_elastic_spectrum(ground_type, pga_value, importance_factor, type, damping_ratio, max_period):
    avg_value, tvb, tvc, tvd = vertical_spectrum_parameter(pga_value, importance_factor, type, ground_type)
    eta = max(0.55, (10 / (5 + damping_ratio / 100)) ** 0.5)
    DATA = []
    mp = max_period
    increment = mp * 1/100
...
def horizontal_design_spectrum(ground_type, pga_value, importance_factor, type, behavior_factor, lower_bound_factor, max_period):
    soil_factor, tb, tc, td = horizontal_spectrum_parameter(pga_value, importance_factor, type, ground_type)
    ag = pga_value * 0.10197 * importance_factor
    DATA = []
    mp = max_period
    increment = mp * 1/100
    # period 리스트 생성
...
def vertical_design_spectrum(ground_type, pga_value, importance_factor, type, behavior_factor, lower_bound_factor, max_period):
    avg_value, tvb, tvc, tvd = vertical_spectrum_parameter(pga_value, importance_factor, type, ground_type)
    DATA = []
    mp = max_period
    increment = mp * 1/100
...
def DS_input(spectrum_type, ground_type, pga_value,  type, importance_factor,damping_ratio, behavior_factor, lower_bound_factor, max_period):
    # period 생성
    p = 0
    per_list = []
    while p <= max_period:
    ...
    if spectrum_type == "Horizontal Elastic Spectrum":
          value = horizontal_elastic_spectrum(ground_type, pga_value, importance_factor, type, damping_ratio, max_period)
    elif spectrum_type == "Vertical Elastic Spectrum":
        value = vertical_elastic_spectrum(ground_type, pga_value, importance_factor, type, damping_ratio, max_period)
    elif spectrum_type == "Horizontal Design Spectrum":
        value = horizontal_design_spectrum(ground_type, pga_value, importance_factor, type, behavior_factor, lower_bound_factor, max_period)
    elif spectrum_type == "Vertical Design Spectrum":
        value = vertical_design_spectrum(ground_type, pga_value, importance_factor, type, behavior_factor, lower_bound_factor, max_period)
...
def generate_description(
    ...
    # 공통 정보
    desc = f"{standard}, {spectrum_type}, Spectrum Type={type}, Ground Type={ground_type}, γI={importance_factor:.2f}, AgR={pga_value:.3f}g"
...
def main_DS_EN1998(
    ...
    # Spectrum data 생성
    inputs = json.loads(DS_input(spectrum_type, ground_type, pga_value, type, importance_factor, damping_ratio, behavior_factor, lower_bound_factor, max_period))
