...
export function get_latidue_longitude(address: string) {	
	return checkPyScriptReady(() => {
		const py_get_lat_long_func = pyscript.interpreter.globals.get('get_latidue_longitude');
		const api_key = "a931e2ae9ed0ae77aaa2c4337f8167b8";		

		console.log('address', address);
		console.log('key', api_key);
		const result = py_get_lat_long_func(address, api_key);
		return JSON.parse(result);
	});
}

export function get_earthquake_coeff(x: string, y: string, scatter_data: number[][], scatter_value: number[]) {	
	return checkPyScriptReady(() => {
		const py_get_eqrthquake_coeff_func = pyscript.interpreter.globals.get('get_earthquake_coeff');
		const result = py_get_eqrthquake_coeff_func(parseFloat(x), parseFloat(y), scatter_data, scatter_value);
		return JSON.parse(result);
	});
}