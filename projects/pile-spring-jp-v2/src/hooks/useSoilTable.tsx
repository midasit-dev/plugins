...
export const useSoilTable = () => {
  ...
  const [tableData, setTableData] = useRecoilState(soilTableData);
  const { groundLevel } = useRecoilValue(soilBasicData); // groundLevel 사용
  const layerTypeItems = SoilLayerTypeItems();

  // layerDepth 재계산 함수
  const recalculateLayerDepths = (
    data: SoilTableRowData[]
  ): SoilTableRowData[] => {
  ...
  const renderRow = (row: SoilTableRowData, index: number): ReactNode[] => [
    ...
    <CustomDropList
      ...
      hideBorder
    />,
    renderNumberField(row, "layerDepth", true), // ✅ 항상 비활성화된 필드
    ...
    renderNumberField(row, "legnth", true),
    <Box
      sx={{
        display: "flex",
