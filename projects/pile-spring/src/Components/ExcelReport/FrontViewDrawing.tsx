...
const FrontViewDrawing = (translate:any, SideLength:any, FoundationWidth:any, PileTableData:any, TopLevel:any, GroundLevel:any) => {
  ...
  const YAmplifyRatio = (canvasSize*0.8)/(TableMaxPileLength+width*0.1)
  ...
  const drawOutlines = (ctx:any) => {
    ...
    ctx.stroke()
    ctx.fillText(`${TableMaxPileLength*1000}`, canvasSize*0.9, canvasSize*0.1+YEndCoordinate+TableMaxPileLength*YAmplifyRatio/2)
    ctx.closePath();

    //저면표고
    ctx.setLineDash([3,2])
    ctx.lineWidth = 0.5
    ctx.moveTo(canvasSize*0.1, canvasSize*0.1+YEndCoordinate)
    ctx.lineTo(canvasSize*0.7, canvasSize*0.1+YEndCoordinate)
    ctx.stroke()
    ctx.fillText(`${translate("Top_Level")} : ${TopLevel}`, canvasSize*0.2, canvasSize*0.1)

    //지반면표고
    const hi = TopLevel - GroundLevel
    ctx.moveTo(canvasSize*0.1, canvasSize*0.1+YEndCoordinate+hi*YAmplifyRatio)
    ctx.lineTo(canvasSize*0.7, canvasSize*0.1+YEndCoordinate+hi*YAmplifyRatio)
    ctx.stroke()
    ctx.fillText(`${translate("Soil_Top_Level")} : ${GroundLevel}`, canvasSize*0.2, canvasSize*0.1+hi*YAmplifyRatio+15)
  };
...
export default FrontViewDrawing;