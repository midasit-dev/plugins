...
function MainWindow() {
  ...
  const [betaNormalResult, setBetaNormalResult] = useRecoilState(Beta_Normal_Result);
  const [betaSeismicResult, setBetaSeismicResult] = useRecoilState(Beta_Seismic_Result);
  const [betaPeriodResult, setBetaPeriodResult] = useRecoilState(Beta_Period_Result);
  const [alphaHThetaResult, setAlphaHThetaResult] = useRecoilState(AlphaHTheta_Result);
  const [kValueNormalResult, setKValueNormalResult] = useRecoilState(KValue_Normal_Result);
  const [kValueSeismicResult, setKValueSeismicResult] = useRecoilState(KValue_Seismic_Result);
  const [kValueSeismicLiqResult, setKValueSeismicLiqResult] = useRecoilState(KValue_Seismic_liq_Result);
  const [kValuePeriodResult, setKValuePeriodResult] = useRecoilState(KValue_Period_Result);
  ...
  const [matrixNormalXResult, setMatrixNormalXResult] = useRecoilState(Matrix_Normal_X_Result);
  const [matrixNormalZResult, setMatrixNormalZResult] = useRecoilState(Matrix_Normal_Z_Result);
  const [matrixSeismicXResult, setMatrixSeismicXResult] = useRecoilState(Matrix_Seismic_X_Result);
  const [matrixSeismicZResult, setMatrixSeismicZResult] = useRecoilState(Matrix_Seismic_Z_Result);
  const [matrixSeismicLiqXResult, setMatrixSeismicLiqXResult] = useRecoilState(Matrix_Seismic_Liq_X_Result);
  const [matrixSeismicLiqZResult, setMatrixSeismicLiqZResult] = useRecoilState(Matrix_Seismic_Liq_Z_Result);
  ...
  const handleCalCulation = () => {
    ...
    // AlphaHTheta 값 계산
    const AlphaHTheta = CalAlphaHTheta(soilData, slopeEffectState, piletableData)
    setAlphaHThetaResult(AlphaHTheta)

    // K1 ~ K4 값 계산 (상시, 지진시, 고유주기)
    const KValue_Normal = CalculateKvalue(piletableData, groundLevel, topLevel, soilData, 'normal', headCondition, bottomCondition, AlphaHTheta, Beta_Normal, Beta_Seismic, Beta_Period, 'no')
    ...
    const Matrix_Seismic_Liq_Z = CalculateMatrix(piletableData, foundationWidth, sideLength, soilData, 'seismic', 'Z', KvResult, KValue_Normal, KValue_Seismic, KValue_Seismic_Liq, KValue_Period, forcePointX, forcePointY, 'yes')
    const Matrix_Period_X = CalculateMatrix(piletableData, foundationWidth, sideLength, soilData, 'period', 'X', KvResult, KValue_Normal, KValue_Seismic, KValue_Seismic_Liq, KValue_Period, forcePointX, forcePointY, 'no')
    const Matrix_Period_Z = CalculateMatrix(piletableData, foundationWidth, sideLength, soilData, 'period', 'Z', KvResult, KValue_Normal, KValue_Seismic, KValue_Seismic_Liq, KValue_Period, forcePointX, forcePointY, 'no')
  ...
  const handleCivilImport = () => {
    ...
    if (importType === 'Type1') {
      ...
      Seismic_Matrix = CombineMatrix(matrixSeismicXResult, matrixSeismicZResult)
      Seismic_Liq_Matrix = CombineMatrix(matrixSeismicLiqXResult, matrixSeismicLiqZResult)
      ...
      Seismic_Matrix = CombineMatrix(matrixSeismicZResult, matrixSeismicXResult)
      Seismic_Liq_Matrix = CombineMatrix(matrixSeismicLiqZResult, matrixSeismicLiqXResult)
    ...
    <GuideBox width="auto">
      ...
      <>
        ...
        <GuideBox width={900} row horRight spacing={1} marginBottom={1}>
          ...
          />
          <Button onClick={handleCalCulation}>{translate("Calculate_Button")}</Button>
          <Button onClick={() => setLoading(true)}>{translate("Export_CalSheet_Button")}</Button>
          <Button onClick={handleCivilImport}>{translate("Import_General_Spring_Button")}</Button>
        </GuideBox>
      </>
    </GuideBox>
  );
