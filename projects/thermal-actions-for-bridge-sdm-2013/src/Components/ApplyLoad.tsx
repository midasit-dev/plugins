...
const ApplyLoad = () => {
  ...
  function ApplyUnifLoad() {
    ...
    if (loadCase_TUH === "" || loadCase_TUC === "") {
      enqueueSnackbar("Please input Load Case Expansion and Contraction.", {
        variant: "error",
      });
      return;
    } else if (loadCase_TUH === loadCase_TUC) {
      enqueueSnackbar(
        "Load Case Expansion and Contraction should be different.",
        { variant: "error" }
      );
      return;
    }
    ...
    // 실행!!!
    // 25-03-20 홍콩 교통부 요청
    // 1.0.0 동작은 선택한 하중 케이스에 온도하중이 있을 경우 ERROR를 송출 (POST 기본 동작)
    // 1.1.0 에서는 PUT으로 업데이트 하자.파이썬 스크립트 수정
    checkPyScriptReady(() => {
      const main = pyscript.interpreter.globals.get(
  ...
  function ApplyDiffLoad() {
    ...
    if (loadCase_TGH === "" || loadCase_TGC === "") {
      enqueueSnackbar("Please input Load Case Heating and Cooling.", {
        variant: "error",
      });
      return;
    } else if (loadCase_TGH === loadCase_TGC) {
      enqueueSnackbar("Load Case Heating and Cooling should be different.", {
        variant: "error",
      });
      return;
    }
  ...
  return (
    <GuideBox width="100%" row horSpaceBetween verCenter>
      ...
      <Button
        ...
        color="negative"
      >
        {tabGroupMain === "Uniform"
          ? "APPLY UNIFORM LOAD"
          : "APPLY DIFFERENTIAL LOAD"}
      </Button>
    </GuideBox>
  );
};
