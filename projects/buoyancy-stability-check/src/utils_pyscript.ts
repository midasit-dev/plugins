...
export function selectElemList(){
	return checkPyScriptReady(() => {
		const py_select_elem_list_func = pyscript.interpreter.globals.get('py_select_elem_list');
		const result = py_select_elem_list_func();
		return JSON.parse(result);
	});
}

export function loadelementinformation(element_id: string, top: string, bottom: string, extWall: string, intStr: string) {
    return checkPyScriptReady(() => {
        const py_select_elementinformation_func = pyscript.interpreter.globals.get('transform_data');
        if (!py_select_elementinformation_func) {
            console.error("Python function 'transform_data' not found.");
            return null;
        }
        const result = py_select_elementinformation_func(element_id, top, bottom, extWall, intStr);
        return JSON.parse(result);
    });
}

export function defaultloadelement() {
    return checkPyScriptReady(() => {
        const py_defaulttransform_data_func = pyscript.interpreter.globals.get('defaulttransform_data');
        if (!py_defaulttransform_data_func) {
            console.error("Python function 'defaulttransform_data' not found.");
            return null;
        }
        const result = py_defaulttransform_data_func();
        return JSON.parse(result);
    });
}
