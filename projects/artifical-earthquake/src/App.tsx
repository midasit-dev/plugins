 ...
 */
const App = () => {
  const [design_spectrum, setDesignSpectrum] =
    useRecoilState(VarDesignSpectrum);
  const design_spectrum_list = useRecoilValue(VarDesignSpectrumList);

  return (
    <GuideBox verTop spacing={2} padding={2}>
      {/* overflow="scroll" */}
        ...
        <GuideBox row spacing={2}>
          <Panel variant="shadow2" padding={2}>
            <GuideBox width={410} height={370} spacing={2} padding={1} verTop>
              <CompTypographyAndDropList
                title="Design Code"
                state={design_spectrum}
                setState={setDesignSpectrum}
                blueTitle
                droplist={design_spectrum_list}
                width={250}
              />
              <DesignSpectrum />
            </GuideBox>
