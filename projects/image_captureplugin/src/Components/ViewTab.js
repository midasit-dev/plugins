...
import { VarModelViewFloorOption, VarPreviewNode, VarPrintImportData, VarViewActive, VarViewActiveElemIds, VarViewActiveGroup, VarViewActiveNodeIds, VarViewActiveUse, VarViewHidden, VarViewViewAngleH, VarViewViewAngleV, VarViewViewType, VarViewViewUse } from "./var";
...
export const ViewTab = React.forwardRef((props, ref) => {
    ...
    const [useActive, setUseActive] = useRecoilState(VarViewActiveUse);
    const [active, setActive] = useRecoilState(VarViewActive);
    const [elemIds, setElemIds] = useRecoilState(VarViewActiveElemIds);
    const [nodeIds, setNodeIds] = useRecoilState(VarViewActiveNodeIds);
    const [selectedGroup, setSelectedGroup] = useRecoilState(VarViewActiveGroup);

    const [useView, setUseView] = useRecoilState(VarViewViewUse);
    const [view, setView] = useRecoilState(VarViewViewType);
    const [angleH, setAngleH] = useRecoilState(VarViewViewAngleH);
    const [angleV, setAngleV] = useRecoilState(VarViewViewAngleV);
    const [hidden, setHidden] = useRecoilState(VarViewHidden);
