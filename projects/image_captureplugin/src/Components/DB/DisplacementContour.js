...
import { VarDispContourComp, VarDispContourLocal, VarDispContourTimeHistoryFunctionType, VarPreviewNode, VarPrintImportData } from "../var";
import { UpdateCurrent } from "./SaveUtils";

export const DisplacementContour = React.forwardRef((props, ref) => {
    const [doUpdate, setDoUpdate] = React.useState("");

    const itemsComp = ["DX", "DY", "DZ", "RX", "RY", "RZ", "RW", "DXY", "DYZ", "DXZ", "DXYZ"];
    const itemsEtc = ["Local (if defined)"];

    const [comp, setComp] = useRecoilState(VarDispContourComp);
    const [local, setLocal] = useRecoilState(VarDispContourLocal);
    ...
    const [timeHistoryType, setTimeHistoryType] = useRecoilState(VarDispContourTimeHistoryFunctionType);
    ...
    const handleSaveClick = () => {
        ...
        if (!copy["DB"])
            copy["DB"] = {};
        copy["DB"]["Deformations - Displacement Contour"] = node;
