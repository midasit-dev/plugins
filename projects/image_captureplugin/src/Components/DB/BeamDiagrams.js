...
import { VarBeamDiagramFidlity, VarBeamDiagramFill, VarBeamDiagramsByMember, VarBeamDiagramScale, VarBeamDiagramsComp, VarBeamDiagramsOuputLocation, VarBeamDiagramsOuputSectionLocation, VarBeamDiagramsPart, VarBeamDiagramsShowTrussForce, VarPreviewNode, VarPrintImportData } from "../var";
...
export const BeamDiagrams = React.forwardRef((props, ref) => {
    const [doUpdate, setDoUpdate] = React.useState("");

    const [part, setPart] = useRecoilState(VarBeamDiagramsPart);

    const itemsComp = ["Fx", "Mx", "Fy", "Fz", "Fyz", "My", "Mz", "Myz", "Mb", "Mt", "Mw",];
    const [comp, setComp] = useRecoilState(VarBeamDiagramsComp);

    const itemsShowTruss = ["Show", "Truss Only"];
    const [showTruss, setShowTruss] = useRecoilState(VarBeamDiagramsShowTrussForce);

    const itemsOutputLocation = ["I", "Center", "J"];
    const [outputLocation, setOutputLocation] = useRecoilState(VarBeamDiagramsOuputLocation);

    const itemsOutputSectionLocation = ["By Section", "Abs Max", "Min/Max", "All"];
    const [outputSectionLocation, setOutputSectionLocation] = useRecoilState(VarBeamDiagramsOuputSectionLocation);

    const itemsByMember = ["By Member"];
    const [byMember, setByMember] = useRecoilState(VarBeamDiagramsByMember);


    const itemsFidelity = ["Exact", "5 Points"];
    const [fidelity, setFidelity] = useRecoilState(VarBeamDiagramFidlity)

    const itemsFill = ["No Fill", "Line Fill", "Solid Fill"];
    const [fill, setFill] = useRecoilState(VarBeamDiagramFill)

    const [scale, setScale] = useRecoilState(VarBeamDiagramScale);
    ...
    const handleSaveClick = () => {
        ...
        const node = {
            ...
            OnlyTrussForces: showTruss.includes("Truss Only"),
        ...
        if (!copy["DB"])
            copy["DB"] = {};
        copy["DB"]["Forces - Beam Diagrams"] = node;
                    ...
                    <GuideBox spacing={1} width={'97%'}>
                        ...
                        <GuideBox row={true} spacing={0.25} width='100%' height="100%">
                            <ListComponent
                                width={'33%'}
                                label={"Fidelity"}
                                userData={itemsFidelity}
                                checkList={fidelity}
                                setCheckList={(l) => setStateUpdate(setFidelity, l)}
                                {...updateKit}
                                singleSelect
                            />
                            <ListComponent
