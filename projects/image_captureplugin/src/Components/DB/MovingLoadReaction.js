...
import { VarMovingLoadReactionComp, VarMovingLoadReactionLocal, VarPreviewNode, VarPrintImportData } from "../var";
import { UpdateCurrent } from "./SaveUtils";

export const MovingLoadReaction = React.forwardRef((props, ref) => {
    const [doUpdate, setDoUpdate] = React.useState("");

    const itemsComp = ["FX", "FY", "FZ", "MX", "MY", "MZ", "Mb"];
    const itemsEtc = ["Local (if defined)"];

    const [comp, setComp] = useRecoilState(VarMovingLoadReactionComp);
    const [local, setLocal] = useRecoilState(VarMovingLoadReactionLocal);
    ...
    const handleSaveClick = () => {
        ...
        if (!copy["DB"])
            copy["DB"] = {};
        copy["DB"]["Moving Load Tracer - Reaction"] = node;
