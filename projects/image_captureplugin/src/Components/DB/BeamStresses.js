...
import { VarBeamStressesCombined, VarBeamStressesComp, VarBeamStressesComp7th, VarBeamStressesOuputLocation, VarBeamStressesOuputSectionLocation, VarBeamStressesPart, VarPreviewNode, VarPrintImportData } from "../var";
...
export const BeamStresses = React.forwardRef((props, ref) => {
    const [doUpdate, setDoUpdate] = React.useState("");

    const [part, setPart] = useRecoilState(VarBeamStressesPart);

    const itemsComp = ["Sax", "Ssy", "Ssz", "Sby", "Sbz", "Combined", "7th DOF"];
    const [comp, setComp] = useRecoilState(VarBeamStressesComp);

    const itemsComp7th = ["Sax(Warping)", "Ssy(Mt)", "Ssy(Mw)", "Ssz(Mt)", "Ssz(Mw)", "Combined(Ssy)", "Combined(Ssz)"];
    const [comp7th, setComp7th] = useRecoilState(VarBeamStressesComp7th);

    const itemsCombined = ["Maximum", "1(-y,+z)", "2(+y,+z)", "4(-y,-z)", "3(+y,-z)"];
    const [combined, setCombined] = useRecoilState(VarBeamStressesCombined);

    const itemsOutputLocation = ["I", "Center", "J"];
    const [outputLocation, setOutputLocation] = useRecoilState(VarBeamStressesOuputLocation);

    const itemsOutputSectionLocation = ["By Section", "Max", "All"];
    const [outputSectionLocation, setOutputSectionLocation] = useRecoilState(VarBeamStressesOuputSectionLocation);
    ...
    const handleSaveClick = () => {
        ...
        if (!copy["DB"])
            copy["DB"] = {};
        copy["DB"]["Stresses - Beam Stresses"] = node;
