...
import { VarMovingLoadBeamStressesComp, VarMovingLoadBeamStressesParts, VarMovingLoadBeamStressesPosition, VarPreviewNode, VarPrintImportData } from "../var";
import { UpdateCurrent } from "./SaveUtils";

export const MovingLoadBeamStresses = React.forwardRef((props, ref) => {
    const [doUpdate, setDoUpdate] = React.useState("");

    const itemsParts = ["i", "1/4", "1/2", "3/4", "j"];
    const [parts, setParts] = useRecoilState(VarMovingLoadBeamStressesParts);

    const itemsComp = ["Combined(Normal)", "Combined(Ssy)", "Combined(Ssz)"];
    const [comp, setComp] = useRecoilState(VarMovingLoadBeamStressesComp);

    const itemsPosition = ["1(-y,+z)", "2(+y,+z)", "3(+y,-z)", "4(-y,-z)"];
    const [pos, setPos] = useRecoilState(VarMovingLoadBeamStressesPosition);
    ...
    const handleSaveClick = () => {
        ...
        if (!copy["DB"])
            copy["DB"] = {};
        copy["DB"]["Moving Load Tracer - Beam Stresses"] = node;
