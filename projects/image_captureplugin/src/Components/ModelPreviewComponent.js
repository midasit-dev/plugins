...
import { VarModelPreviewNode, VarModelPreviewSelected } from "./var";
...
const ModelPreviewComponent = () => {
    const [bp_Node, setBP_Node] = useRecoilState(VarModelPreviewNode);
    const [selectedLabel, setSelectedLabel] = useRecoilState(VarModelPreviewSelected);
    ...
    const handleNodeSelect = (event, itemId) => {
        ...
        const firstPart = itemId.split('-')[0];
        setSelectedLabel(firstPart);
        console.log("Selected label:", firstPart);
    };
        ...
        <GuideBox verSpaceBetween show padding={2} spacing={1.5} width={'100%'}>
            ...
            >
                <Scrollbars
                    ...
                    autoHide
                >
                    <SimpleTreeView defaultExpandedItems={['root']} onItemSelectionToggle={handleNodeSelect} >
...
<MoaStack width={'100%'} direction="row" justifyContent={"space-between"} spacing={1}>
    {/* <div style={{ flex: 1 }}>
        <AddFileButton onAfterUpload={handleAfterUpload} buttonName="Import Json" />
    </div> */}
    <div style={{ flex: 1 }}>
        <DownloadButton2 valueToDownload={bp_Node} buttonName="MODEL View Export Json" />
    </div>
</MoaStack>
        </GuideBox>
    );
...
export default ModelPreviewComponent;